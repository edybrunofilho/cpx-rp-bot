...
import {supportCommands} from './support-interactions.mjs';
const e=process.env;
...
for(const command of [commandDefinition,...ownerCommands,...supportCommands,...cnhExamCommands,...rgCommands])await discord('/applications/'+e.DISCORD_CLIENT_ID+'/guilds/'+e.DISCORD_GUILD_ID+'/commands',e.DISCORD_BOT_TOKEN,{method:'POST',body:JSON.stringify(command)});
...
console.log('Comandos CPX, /ticketpainel, /criar e /cnh registrados no servidor original. /staff e /player permanecem em '+STAFF_GUILD_ID+'. /ver e /banco foram registrados somente em '+BANK_GUILD_ID+'.');

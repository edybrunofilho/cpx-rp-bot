...
export const roleNames = { citizen:'Cidadão', admin:'Administrador', mayor:'Prefeito', deputy:'Vice-prefeito', government:'Governo' };
...
export function canTreasury(role,key){return role==='admin'||(key==='city'&&['mayor','deputy'].includes(role))||(key==='state'&&role==='government');}
export function allowedChannels(role){return ['comunidade',...(role==='admin'?['prefeitura','administracao','banco','governo']:role==='mayor'||role==='deputy'?['prefeitura']:role==='government'?['governo']:[])];}
export function applyAction(state,actor,input){
 const me=state.players.find(p=>p.id===actor.id);if(!me)fail('Entre na sua conta para continuar.',401);
 if(guardianActions.includes(input.action))return applyGuardian(state,actor,input);
 const a=input.action;const events=[];
 if(a==='transfer'||a==='adjust'||a==='treasury'){
  ...
  const tx={id:crypto.randomUUID(),requestId:input.requestId,from,to,amount,reason,actor:me.name,at:new Date().toISOString(),dm:actor.demo?'Demonstração':'Na fila'};state.transactions.unshift(tx);events.push(tx);
 }else if(a==='profile'){
